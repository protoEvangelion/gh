...
PullRequest.prototype.printPullsInfoTable_ = function(pulls) {
    ...
    function generateTable() {
        ...
        tableHead = tableHead.map(heading => {
            if (typeof heading === 'string') {
                return logger.colors.bold.red(heading)
            }

            return { content: logger.colors.bold.red(heading.content), hAlign: heading.hAlign }
        })
        ...
        pulls.forEach(function(pull) {
            ...
            const number = logger.colors.bold.green(`#${pull.number}`)
    ...
    function formatPrInfo(pull, length) {
        ...
        if (showDetails) {
            ...
            info = `
                ${logger.colors.bold.blue('Title:')}

                ${title}

                ${logger.colors.bold.blue('Body:')}
...
PullRequest.prototype.printPullInfo_ = function(pull) {
    ...
    } else if (pull.mergeable_state !== undefined && !testing) {
